...
$SQL = "SELECT 
    ...
    e.observacion, e.foto,
    c5.pais AS PaisTrabajo
FROM 
    estudiantes e
JOIN 
...
$SQL2 = "SELECT nombre FROM paises WHERE codigo = :codigo";
$Sentencia2 = $BaseDatos->Conexion->prepare($SQL2);
$Sentencia2->bindValue(":codigo", $registro[20]);
$Sentencia2->execute();  //Ejecuta la consulta
$registro2 = $Sentencia2->fetch();

//Respuesta HTML
$Pantalla = file_get_contents("../../visual/estudiantes/detalle.html");
$Pantalla = str_replace("{codigo}", $registro[0], $Pantalla);
$Pantalla = str_replace("{nombre1}", htmlentities($registro[1], ENT_QUOTES, "UTF-8"), $Pantalla);
$Pantalla = str_replace("{nombre2}", htmlentities($registro[2], ENT_QUOTES, "UTF-8"), $Pantalla);
$Pantalla = str_replace("{apellido1}", htmlentities($registro[3], ENT_QUOTES, "UTF-8"), $Pantalla);
$Pantalla = str_replace("{apellido2}", htmlentities($registro[4], ENT_QUOTES, "UTF-8"), $Pantalla);
...
$Pantalla = str_replace("{correo}", htmlentities($registro[13], ENT_QUOTES, "UTF-8"), $Pantalla);
$Pantalla = str_replace("{url}", htmlentities($registro[14], ENT_QUOTES, "UTF-8"), $Pantalla);
...
$Pantalla = str_replace("{observacion}", $registro[18], $Pantalla); //htmlentities (?)
$Pantalla = str_replace("{foto}", "<img src='$registro[19]'></img>", $Pantalla); //htmlentities (?)
$Pantalla = str_replace("{paistrabaja}", $registro2[0], $Pantalla);
echo $Pantalla;
